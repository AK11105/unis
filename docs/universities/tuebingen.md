---
tags:
  - Germany
  - Target
---

# [University of Tübingen](https://uni-tuebingen.de/en/)

> **Country:** Germany | **Tier:** Target

=== "Crux"

    | | |
    |---|---|
    | **Cost** | ~€6.7K total · €800–1,100/mo |
    | **Deadline** | 30 April |
    | **GRE** | Not required |
    | **English** | English required (verify scores) |
    | **Duration** | 4 semesters · 120 ECTS · English-taught |

    ---

    **Why it's here:** Arguably the single strongest ML location in continental Europe. The Cluster of Excellence in ML, Cyber Valley, Max Planck Institute for Intelligent Systems, and ELLIS unit are all here. Amazon Research and Bosch Center for AI are nearby. This programme exists specifically to produce ML researchers — if that's your path, nowhere else comes close.

    **Programmes:** Machine Learning

    **Watch out:** €1,500/sem BW tuition. Small city (focused research environment, not a metropolis). ML M.Sc. is the English option — Informatik is German-taught.

=== "Detail"

    ## Overview

    ### Rankings

    - One of the world's leading centres for Machine Learning research
    - Home to the **Cluster of Excellence "Machine Learning: New Perspectives for Science"**
    - **Cyber Valley** — Europe's largest AI research consortium
    - ELLIS Unit Tübingen — member of the European Laboratory for Learning and Intelligent Systems

    ### Tuition (Non-EU International Students)

    - **€1,500/semester** (Baden-Württemberg regulation for non-EU students)
    - **Semester contribution:** additional ~€150/semester
    - **Total estimated cost (4 semesters):** ~€6,600

    ### Estimated Living Cost

    - €800–1,100/month (Tübingen is a small university town, moderate cost)

    ### Scholarships

    - DAAD
    - Deutschlandstipendium (€300/month)
    - Cyber Valley Graduate Program scholarships
    - External foundations

    ### Research Strengths

    - **Machine Learning** (world-class — Cluster of Excellence)
    - Deep Learning
    - Computer Vision
    - Causal Inference
    - Reinforcement Learning
    - Probabilistic ML
    - Representation Learning
    - Autonomous Systems
    - Robotics
    - Theoretical CS

    ### Industry & Research Connections

    - **Cyber Valley partners:** Max Planck, Bosch, Amazon, Porsche, BMW, ZF, IAV
    - Max Planck Institute for Intelligent Systems
    - Amazon Research Tübingen
    - Bosch Center for AI

    ---

    ## Target Programs

    ### Machine Learning (M.Sc.)

    | Field | Detail |
    |-------|--------|
    | Duration | 4 semesters |
    | Credits | 120 ECTS |
    | Language | **English** |
    | Start | Winter Semester |
    | Thesis | Mandatory |

    #### Highlights

    - Fully English-taught international programme
    - World-class ML faculty (home of many top ML researchers)
    - Direct access to Cluster of Excellence research
    - Cyber Valley integration
    - Covers all major ML areas: deep learning, probabilistic ML, causal inference, RL, representation learning

    #### Admission

    - Bachelor's in CS, Mathematics, Physics, or closely related field
    - Strong mathematical foundations required
    - English proficiency required
    - Competitive admission

    #### Deadline

    - **30 April** (verify annually on official portal)

    ---

    ### Computer Science / Informatik (M.Sc.)

    | Field | Detail |
    |-------|--------|
    | Duration | 4 semesters |
    | Credits | 120 ECTS |
    | Language | German (primarily) / some English courses |
    | Start | Winter or Summer Semester |

    #### Deadline

    - Likely 15 July for WS (standard BW deadline)

    ---

    ## Admission Requirements

    ### Academic Requirements

    - Bachelor's degree (≥180 ECTS) in Computer Science, Mathematics, Physics, or related field
    - Strong mathematical background (linear algebra, calculus, probability, statistics)
    - Programming experience

    ### English Language Requirement

    - English proficiency required for ML M.Sc.

    ### GRE

    - Not explicitly required (based on available information)

    ### APS Certificate

    - Required for Indian applicants (standard for German universities)

    ---

    ## Application

    ### Application Deadlines

    | Programme | Deadline |
    |-----------|----------|
    | Machine Learning M.Sc. | **30 April** (verify annually) |
    | Informatik M.Sc. | ~15 July (WS) |

    ### Application Platform

    - **[Tübingen Online Portal](https://uni-tuebingen.de/en/study/application/)**


    ---

    ## Tuition & Costs

    | Item | Amount |
    |------|--------|
    | Tuition (Non-EU) | €1,500/semester |
    | Semester contribution | ~€150–200/semester |
    | Living costs | €800–1,100/month |
    | Total tuition (4 semesters) | €6,000 |
    | Total fees (4 semesters) | ~€6,600–€6,800 |

    ---

    ## Research

    ### The Tübingen ML Ecosystem

    Tübingen is arguably the single strongest location for ML research in continental Europe:

    - **Cluster of Excellence "Machine Learning: New Perspectives for Science"**
    - **Max Planck Institute for Intelligent Systems** — Empirical Inference, Autonomous Motion, Haptic Intelligence
    - **Cyber Valley** — Europe's largest AI consortium (Max Planck + Universities + Industry)
    - **ELLIS Unit Tübingen** — European Laboratory for Learning and Intelligent Systems
    - **Amazon Research Tübingen**
    - **Bosch Center for AI (nearby Stuttgart)**

    ### Key Research Areas

    - Deep Learning Theory
    - Probabilistic Machine Learning
    - Causal Inference & Discovery
    - Computer Vision
    - Reinforcement Learning
    - Representation Learning
    - Robotics & Autonomous Systems
    - Generative Models
    - ML for Science (Physics, Biology, Climate)

    ---

    ## Notes & Sources

    === "Notes"

        - Tübingen's ML M.Sc. is one of the most research-oriented ML programmes in Europe.
        - The ML ecosystem (Cluster of Excellence + Max Planck + Cyber Valley + ELLIS) is unmatched in Germany.
        - The city is small and charming but not a metropolis. Ideal for focused research.
        - Baden-Württemberg tuition (€1,500/semester) applies — same as KIT.
        - Application deadline (~April) is much later than most other universities on this list.
        - Ideal for students specifically targeting ML/AI research careers and PhD pathways.
        - Amazon Research and Bosch Center for AI provide industry thesis/internship opportunities.
        - The Informatik M.Sc. is primarily German-taught; ML M.Sc. is the English-taught option.

    === "Sources"

        - [Machine Learning M.Sc.](https://uni-tuebingen.de/fakultaeten/mathematisch-naturwissenschaftliche-fakultaet/studium/satzungen/machine-learning-msc/)
        - [Computer Science M.Sc.](https://uni-tuebingen.de/studium/studienangebot/verzeichnis-der-studiengaenge/detail/course/informatik-computer-science-master/)
        - [Cluster of Excellence ML](https://uni-tuebingen.de/forschung/forschungsschwerpunkte/exzellenzcluster-maschinelles-lernen/)
        - [Cyber Valley](https://cyber-valley.de/en)
        - [International programmes](https://uni-tuebingen.de/international/studierende-aus-dem-ausland/degree-seeking-students/studienangebot-fuer-internationale-studierende/internationale-studiengaenge/)
